...
function AllOrders() {
  ...
  useEffect(() => {
    const fetch = async ()=>{
      const response = await axios.get("http://localhost:1010/api/order/getallOrder",
        {headers}
      )
  ...
  const submitChanges = async (i) =>{
    ...
    const response = await axios.put(`http://localhost:1010/api/order/updateOrder/${id}`,
      Values,
      { headers }
    )
        ...
        <div className='h-[100%] p-0 md:p-4 text-zinc-100'>
          ...
          <div className='mt-4 bg-zinc-800 w-full rounded py-2 px-4 flex gap-2 '>
           ...
           {AllOrders && AllOrders.map((items,i) =>(
            <div className='bg-zinc-800 w-full rounded py-2 p-4 flex gap-2 hover:bg-zinc-900 hover:cursor-pointer transition-all duration-1s'>
                <div className='w-[3%]'>
                  <h1 className='text-center'>{ i+ 1}</h1>
                </div>
                <div className='w-[40%] md:w-[22%]'>
                  <Link
                       to={`/ViewBookDetails/${items.book._id}`} className='hover:text-blue-300'>
                        {items.book.title}
                  </Link>
                </div>
                <div className='w-0 md:w-[45%] hidden md:block'>
